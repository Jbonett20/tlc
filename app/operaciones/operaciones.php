...
if ($variable == 'ingresos') {
	if ($operacion == "UpdateNombreFacturaSelect") {
		...
		mysqli_close($link);
	}  else if ($operacion == 'listausuarios') {

		$sql_busqueda = mysqli_query($link,"SELECT * FROM tbl_usuario_sistema");
		$rows = [];
		while ($respuesta = mysqli_fetch_assoc($sql_busqueda)) {
			$rows[] = $respuesta;
		}
		...
		// SELECT p.*, sum(fcd.cantidad) as Cantidad,fcd.fechaCaducidad from tbl_fechacaducidaddetalle fcd,tbl_fechacaducidad fc,tbl_producto p WHERE fcd.id_fechaCaducidad=fc.id_fechaCaducidad and p.id_producto=fc.id_producto and fcd.fechaCaducidad=fcd.fechaCaducidad
		$sql_busqueda = mysqli_query($link,"SELECT p.*,fcd.* from tbl_fechacaducidaddetalle fcd,tbl_fechacaducidad fc,tbl_producto p WHERE fcd.id_fechaCaducidad=fc.id_fechaCaducidad and p.id_producto=fc.id_producto and fcd.fechaCaducidad=fcd.fechaCaducidad and p.id_producto='$id' ORDER BY fcd.fechaCaducidad ASC");
		$rows = [];
		while ($respuesta = mysqli_fetch_assoc($sql_busqueda)) {
			$rows[] = $respuesta;
		}
	...
	if ($operacion == "EliminarcaducidadFactura") {
		...
		if (!$sql_listadoProducto) {
			...
			$sql_eliminarFecha = mysqli_query($link,"DELETE FROM tbl_fechacaducidaddetalle  WHERE id_fechaCaducidad='$id'");
			if(!$sql_eliminarFecha){
				echo "fallo2";
			}else{
				echo "exito";
			}
	...
	if ($operacion == 'insertarplansepare') {
		...
		if (floor($stock) >= floor($fraccion) && floor($fraccion)>0) {
			$NSumaFraccion = floor($stock) / floor($fraccion);
			$stock = floor($stock) % floor($fraccion);
		...
		$sql_listadoClientes_factura =  mysqli_query($link,"SELECT p.*,p.presentacion,i.Unidad,p.descripcion,p.fraccion,p.unidadCerrada,p.codigo_producto,c.nombre_categoria,i.stock,i.* FROM tbl_producto p,tbl_categoria c, tbl_inventario i WHERE i.id_producto=p.id_producto and p.id_categoria=c.id_categoria ORDER BY p.id_producto ASC");
	...
	} else if ($operacion == 'facturaListaDatDiario')
	{
		
		$fechaHoy= date("Y-m-d");

		$sql_listado_facturaxfecha = mysqli_query($link,"SELECT * FROM tbl_basediaria WHERE fecha='$fechaHoy'");	
			...
			if ($fraccionProducto == 0) {
				...
				if (!$sql_updateInventarios) {
				    ...
				    try {
                        ...
                        mysqli_query($link, "UPDATE tbl_factura SET valor_pago='$valorUp' WHERE id_factura='$id_factura'");
                        mysqli_query($link, "DELETE FROM tbl_detallefactura WHERE id_detalleFactura='$id_detallefactura'");
                        ...
                        if (mysqli_num_rows($sql_busquedaDevolucion) == 0) {
                            mysqli_query($link, "INSERT INTO tbl_devolucion VALUES (NULL, '$var_codigo_factura', '{$arr['id_empresa']}', CURRENT_DATE(), '$hora', '{$arr['id_cliente']}', '{$arr['valor_pago']}', '{$arr['pagoCambio']}', '{$arr['cambio']}', '{$arr['id_vendedor']}')");
                        } else {
                            $sql_busquedadetalle = mysqli_query($link, "SELECT * FROM tbl_devolucion WHERE codigo_factura='$var_codigo_factura'");
                            $arryC = mysqli_fetch_array($sql_busquedadetalle);
                            $valorUpC = $arryC['valor_pago'] + $pagoProducto;
                            mysqli_query($link, "UPDATE tbl_devolucion SET valor_pago='$valorUpC' WHERE codigo_factura='$var_codigo_factura'");
                        }
                        ...
                        mysqli_query($link, "INSERT INTO tbl_detalledevoluciones VALUES (NULL, '{$arry['id_devolucion']}', '$id_producto', '$cantidadUnidad', '$cantidadFraccion', '$pagoProducto', '$idUs', 'Devolucion', CURRENT_DATE(), '$hora')");
                        
                        echo "✅ Proceso completado correctamente";
                    
                    } catch (Exception $e) {
                        echo "<pre>❌ Error detectado: " . $e->getMessage() . "</pre>";
                    }
		...
		if (!$sql_detallefactura) {
			...
			echo "exito";
			// $sql_cantidadinvetario = mysqli_query($link,"SELECT * FROM tbl_inventario WHERE id_producto='$id_producto' ",$link);
			// 	$filainvetario = mysqli_fetch_array($sql_cantidadinvetario);
			// 	$stock = $filainvetario['stock'];

			// 	$nuevoStock = $stock - $cantidad;

			// $sql_cantidadinvetario = mysqli_query($link,"UPDATE tbl_inventario SET stock='$nuevoStock' WHERE id_producto='$id_producto'",$link);
...
if ($variable == "categoria") {
	...
	if ($operacion == 'insertarBaseDiaria') {
		...
		$Insert_categoria = mysqli_query($link,"INSERT INTO tbl_basediaria VALUES (null,'$baseDiaria',current_date() )");
		if (!$Insert_categoria) {
			echo "fallo";
		} else {
			echo "exito";
	...
	} else if ($operacion == 'listadodeBaseDiaria')
	{
		...
		echo $rows = json_encode($rows);
		mysqli_close($link);
	}else if ($operacion == 'listadodeBaseDiariaHoy')
	{
		$fechaHoy= date("Y-m-d");
		$sql_todoCategoria =  mysqli_query($link,"SELECT * FROM  tbl_basediaria WHERE fecha='$fechaHoy'");
...
if ($variable == "egreso") {
	...
	if ($operacion == 'egreso') {
		...
		$Insert_categoria = mysqli_query($link,"INSERT INTO  tbl_egresos VALUES (null,'$idtipoEgreso','$pagado','$valorEgreso',current_date(),'$mes',$idVendedor)");
		if (!$Insert_categoria) {
			echo "fallo";
		} else {
			echo "exito";
...
if ($variable == "producto") {
	...
	if ($operacion == 'insertarProveedor') {
		...
		$sql_listadoProducto = mysqli_query($link,"SELECT * FROM tbl_producto p,tbl_inventario i WHERE p.id_producto=i.id_producto  order by p.id_producto  ");
		...
		// $fechaHoy= date("Y-m-d");
		$sql_listadoProducto = mysqli_query($link,"SELECT inv.*,iv.*,pro.*,pro.valor_venta*iv.iva/100 as ivaValor,pro.editableValor,pro.id_producto,SUM(f.descuento) as descuentoTo,df.descuento,f.codigo_factura,f.fecha_factura,f.valor_pago,pro.fraccion,pro.descripcion,SUM(df.total_pago) as TotalPago,f.ganacia,SUM(df.cantidad)  as cantidad,SUM(df.cantidadFraccion) as cantidadFraccion  FROM tbl_factura f, tbl_empresa em,tbl_detallefactura df, tbl_producto pro,tbl_iva iv,tbl_inventario inv WHERE f.id_empresa=em.id_empresa and df.id_factura=f.id_factura and inv.id_producto=pro.id_producto and iv.id_iva=pro.id_iva and df.id_producto=pro.id_producto and f.fecha_factura='$fechaAtras'  GROUP BY(pro.descripcion) ORDER BY cantidad DESC limit 10");
		...
		if ($valorVentaProducto == 0) {
			...
			$ventaUn = (isset($valorUnidadProducto) && $valorUnidadProducto > 0) ? $valorUnidadProducto : ($valorVentaProducto / $fraccion);
			$costoUn = ($valorProducto / $fraccion);
			if ($ventaUn == 0) {
				$rentabilidad = 0;
			} else {
				$rentabilidad = round((($ventaUn - $costoUn) / $ventaUn) * 100, 2);
			}
		} else {
			$rentabilidad = round((($valorVentaProducto - $valorProducto) / $valorVentaProducto) * 100, 2);
		}
...
// ===========================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================
	...
	if ($operacion == 'insertarcredito') {
		...
		$valorDescuento = $filacreditocalores['valorDescuento'];
		$valorDescuentoDinero = $totalapagar_plansepare * $valorDescuento / 100;

		$sumapago = $valor_aumento + $totalapagar_plansepare;
		$totalapagar = $sumapago - $valorDescuentoDinero;
		$idVendedor = (int)$_SESSION['id'];
		$sql_ultimacredito = mysqli_query($link,"SELECT * FROM tbl_credito order by id_credito DESC limit 1 ");
		$filacredito = mysqli_fetch_array($sql_ultimacredito);
		...
		$estadoproducto = "No Entregado";
		$sql_insertar_credito = mysqli_query($link,"INSERT INTO tbl_credito VALUES (null,'$codigocredito','$id_cliente','$valor_aumento','$sumapago','$sumapago','$sumapago','$fecha_inicioPlansepare','$fechaFin_plansepare','$estadoproducto','$idVendedor')");
		...
		echo $rows = json_encode($rows);
		mysqli_close($link);
	} else if ($operacion == 'busquedaCreditoListoVencidas') {
		$fechaHoy = date("Y-m-d");
		$contador = 0;
		$sql_verificarcreditosValores = mysqli_query($link,"SELECT * FROM tbl_creditovalores");
		$request = mysqli_fetch_assoc($sql_verificarcreditosValores);
		$sql_verificarcreditos = mysqli_query($link,"SELECT * FROM tbl_credito");
		while ($respuesta = mysqli_fetch_assoc($sql_verificarcreditos)) {

			// if ($respuesta['fecha_final'] < $fechaHoy) {

			// 	$request['valorDisminucion'];
			// 	$request['valorDisminucion'];
