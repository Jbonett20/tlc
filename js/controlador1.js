...
$scope.guardarIngresosProducto  = function (id,AgregarCantidadProducto)
	{
		...
		{		
			...
			else if (datos == 'guardo')
			{
				...
				$scope.listadotodos_ProductoChange($scope.barras);
...
if ($scope.listadodetodos_ProductoC[0].fraccion!=0) {
   ...
   //      	agregarInsertProduFactura.id_productoFactura = '';
	...
	$scope.agregarProductoFactura = function(elEvento,id_producto,descripcion,codigo_producto,valor,valor_venta,ivaV,iva,presentacion,fraccion,valor_unidad,cant,editableValor)
	{
		elEvento,
		 evento = elEvento || window.Event;
         k=evento.keyCode; //número de código de la tecla. para el enter debe ser 13
...
if (contador!=0) {
	...
	console.log(valorEnPorcentaje);
	cambioFacturaDinero.descuento=cambioFacturaDinero.descuento + "("+valorEnPorcentaje+"%)"
...
// SELECT * FROM tbl_producto pro, tbl_factura fac, tbl_detallefactura def, tbl_iva iva WHERE def.id_factura=fac.id_factura AND def.id_producto=pro.id_producto AND pro.id_iva=iva.id_iva AND iva.iva=0
		...
		console.log(cambioFacturaDinero.tipoPago);
...
$scope.agregarbusquedaFacturaXf={
	...
	$scope.buscarfacturaXfecha = function(agregarbusquedaFacturaXf)
	{
		...
		console.log(typeof(agregarbusquedaFacturaXf.fechaFinalFactura));

		if (typeof(agregarbusquedaFacturaXf.fechaFinalFactura)=="string") {
			console.log("final vacio");
		...
		{		
			...
			else
			{
				...
				$scope.paginalistadofacturasXfechasDos = function() {
				  ...
				  console.log($scope.pagelistadofacturasXfechasDos);
				}
			}
	...
	$scope.visibleDateDos=false;
			...
			$scope.totalUnidadDes = 0;
				...
				$scope.paginalistadoDescuadreXunidad = function() {
				  ...
				  console.log($scope.pagelistadoDescuadreXunidad);
				}
			}
	...
	$scope.visibleDateDos=true;
				...
				$scope.paginalistadoDescuadreXunidadNomodificado = function() {
				  ...
				  console.log($scope.pagelistadoDescuadreXunidadNomodificado);
				}
			}
...
$scope.listadotodos_IngresosFacturaAdd  = function ()
	{
		...
		{		
			...
			$scope.itemslistadodetodos_addFactura = $scope.listadodetodos_addFactura.slice(0, 4);
			
			$scope.paginalistadodetodos_addFactura = function() {
			  var startPos = ($scope.pagelistadodetodos_addFactura - 1) * 4;
...
console.log($scope.id_fechacaducidad);
}).error(function(datoss, status, headers, config)
		{
	      //	console.log("echo todo mal");
	      	new PNotify({
...
$scope.guardarDetalleFechaCaducidad= function(insertarFechaCaducidad,id_fechacaducidad){
console.log(insertarFechaCaducidad.lote);
...
$scope.listaDetallefechaCaducidadLista=datoss;
			...
			$scope.paginalistaDetallefechaCaducidadLista = function() {
			  ...
			  console.log($scope.pagelistaDetallefechaCaducidadLista);
			}
...
console.log(id_producto,descripcion,presentacion);
...
$http.post("app/operaciones/operaciones.php?variable=producto&operacion=insertarSeccionD",insertarSeccion).success(function (datos) {
		...
		{		
			...
			$scope.paginalistadodetodos_Cateoria = function() {
			  ...
			  console.log($scope.pagelistadodetodos_Seccion);
			}
	...
	$scope.listadotodos_Cajas  = function ()
	{
		...
		{		
			...
			$scope.paginalistadodetodos_Cateoria = function() {
			  ...
			  console.log($scope.pagelistadodetodos_Caja);
			}
...
{
	if (insertarProducto.unidadCerra<0 || insertarProducto.fraccion<0 || insertarProducto.valorProducto<0 || insertarProducto.valorProductoUnidad<0 || insertarProducto.valorVentaProducto<0 || insertarProducto.valorVentaProductoUnidad<0 || insertarProducto.stockMin<0  ) {
...
if (insertarProducto.fraccion==0) {
 divicion=0;
	...
	if ( insertarProducto.fraccion !=0 && insertarProducto.valorProducto >  insertarProducto.valorVentaProducto|| divicion > insertarProducto.valorVentaProductoUnidad ) {
		...
		console.log(insertarProducto.fraccion);
		console.log(insertarProducto.valorProducto);
		console.log(insertarProducto.valorVentaProducto);
		console.log(insertarProducto.valorVentaProductoUnidad);
			new PNotify({
					title: 'Oh No!',
...
$scope.listadoPorducto_Keypress  = function (elEvento,elcodigoProd)
	...
	new PNotify({
			...
			valorProductoUnidad:parseInt(datoss[0].valor_venta),
	...
	$http.post("app/operaciones/operaciones.php?variable=producto&operacion=listadodeProductosChangeLetra",{
		...
		.success(function (datoss)
		{
			...
			$scope.paginalistadodetodos_ProductoModProd = function() {
			  ...
			  console.log($scope.pagelistadodetodos_ProductoModProd);
			}
...
$scope.listadoPorOrdenProducto  = function ()
	{
		...
		console.log(datoss);
			...
			$scope.itemslistadodetodos_ProductoModProd = $scope.listadodetodos_ProductoModProd.slice(0, 100);
			
			$scope.paginalistadodetodos_ProductoModProd = function() {
			  var startPos = ($scope.pagelistadodetodos_ProductoModProd - 1) * 10000;
				...
				for (var b = 0; b < $scope.listaProductosPlan_separe.length; b++)
				{
					...
					{		
						...
						if (respuesta == 'exito')
						{
							...
							$scope.listaProductosPlan_separe="";
							insertarplansepare.id_cliente="";
							insertarplansepare.fecha_inicioPlansepare="";
							insertarplansepare.fechaFin_plansepare="";
							insertarplansepare.valor_aumento=0;
							insertarplansepare.identificacion="";
							insertarplansepare.nombre_cliente="";
							$scope.totalapagarSepare=0;
							new PNotify({
								title: 'Exito!',
								text: 'Se ha guardado con exito el registro de los productos',
...
for (var i = 0; i < $scope.delDia.length; i++)
					...
					totaltotal=totaltotal+parseInt($scope.delDia[i].TotalPago);
							...
							console.log($scope.delDia.length);
