...
$scope.agregarProductoFacturaChange = function(numeroPro,id_producto,descripcion,codigo_producto,valor,valor_venta,ivaV,iva,presentacion,fraccion,valor_unidad,cantidadU,cantidadF,editableValor)
{
				...
				{	
					if ($scope.listaProductosDetalleFactura == '') 
					{
						...
						// {
							...
							$scope.listaProductosDetalleFactura.push(
							{
								numeroPro:$scope.numeroPro, id_producto: id_producto,codigoProducto: codigo_producto, descripcion: descripcion, cantidadU: cant.cantidadU,cantidadF:cant.cantidadF, valor:valor, valor_venta: valor_venta,iva: iva,ivaV: ivaV,presentacion: presentacion, valorTotal: totalvalorXcantidad, fraccion: fraccion,ganacia:ganacia}
							);
							$scope.numeroPro++;
							}
...
$scope.cambioFacturaDinero={
	...
	tipoPago:"EFECTIVO",
	tipo_factura:'1' // Por defecto factura normal
...
// SELECT * FROM tbl_producto pro, tbl_factura fac, tbl_detallefactura def, tbl_iva iva WHERE def.id_factura=fac.id_factura AND def.id_producto=pro.id_producto AND pro.id_iva=iva.id_iva AND iva.iva=0
		...
		// Determinar el tipo de factura (default es 1 si no está especificado)
		$scope.tipo_factura_actual = cambioFacturaDinero.tipo_factura ? parseInt(cambioFacturaDinero.tipo_factura) : 1;
		
		console.log('🔍 DEBUG - cambioFacturaDinero.tipo_factura:', cambioFacturaDinero.tipo_factura);
		console.log('🔍 DEBUG - tipo_factura_actual:', $scope.tipo_factura_actual);
		
		// Seleccionar la operación según el tipo de factura
		var operacion = ($scope.tipo_factura_actual == 2) ? 'insertarfacturaElectronica' : 'insertarfactura';
		
		console.log('🔍 DEBUG - operacion seleccionada:', operacion);
		console.log('🔍 DEBUG - datos a enviar:', {
			id_cliente: insertClientes.id_cliente,
			tipo_factura: $scope.tipo_factura_actual
		});
		
		$http.post("app/operaciones/operaciones.php?variable=facturar&operacion=" + operacion,
		{
			'id_cliente':insertClientes.id_cliente,
	...
	$scope.guardarFacturaElec = function(insertClientes,cambioFacturaDinero)
	{
		...
		if ($scope.listaProductosDetalleFactura!="" && cambioFacturaDinero.cambio>=0 ) {
			...
			{		
				...
				// Abrir ventana en blanco para evitar bloqueo de popup y mostrar la factura cuando esté lista
				var ventanaFactura = window.open('views/informespdf/BaseFactura.php?factura=' + id_factura, '_blank');
				if (!ventanaFactura) {
					console.log('Advertencia: popup bloqueado o no permitido por el navegador.');
				}

				// SOLO LLAMAR A MOVIMIENTOAPI SI ES FACTURA ELECTRÓNICA (tipo_factura = 2)
				if ($scope.tipo_factura_actual == 2) {
							...
							if (resMovimiento.success === true) {
								...
								if (resMovimiento.data && resMovimiento.data.movimiento && resMovimiento.data.movimiento.faencmovi) {
									...
									if (ventanaFactura && !ventanaFactura.closed) {
										ventanaFactura.location = 'views/informespdf/BaseFactura.php?factura=' + id_factura + '&cufe=' + cufe + '&qr=' + encodeURIComponent(qrUrl);
									} else {
										window.open('views/informespdf/BaseFactura.php?factura=' + id_factura + '&cufe=' + cufe + '&qr=' + encodeURIComponent(qrUrl), '_blank');
									}
								} else {
									if (ventanaFactura && !ventanaFactura.closed) {
										ventanaFactura.location = 'views/informespdf/BaseFactura.php?factura=' + id_factura;
									} else {
										window.open('views/informespdf/BaseFactura.php?factura=' + id_factura, '_blank');
									}
								}
				...
				// Factura normal: no llamar a Movimientoapi, solo mostrar el PDF
				console.log('Factura normal - no se enviará a DIAN');
			}
		...
		$http.post("app/operaciones/operaciones.php?variable=facturar&operacion=busquedaDetallefacturaselecionadaDevolucion",
			...
			}).error(function(respuesta, status, headers, config)
			{
		      ...
		      //	console.log("echo todo mal");
		      	new PNotify({
						title: 'Error!',
						text: 'Ha ocurrido un error de funcion Devolución',
						type: 'error',
						styling: 'bootstrap3'
					});
		    });
...
$http.post("app/operaciones/operaciones.php?variable=producto&operacion=insertar",insertarProducto).success(function (datos) {
		...
		// Verificar si la respuesta es JSON o texto plano (compatibilidad con versiones antiguas)
		var respuesta = datos;
		if (typeof datos === 'string') {
			try {
				respuesta = JSON.parse(datos);
			} catch (e) {
				// Si no es JSON válido, usar el formato antiguo
				respuesta = { success: false, code: datos };
			}
		}

		// Manejar respuesta en formato JSON (nuevo)
		if (respuesta.success === true) {
			new PNotify({
				title: 'Éxito!',
				text: respuesta.message || 'Producto guardado exitosamente',
				type: 'success',
				styling: 'bootstrap3'
			});
			$scope.listadoPorOrdenProductolist();
		...
		// Manejar errores en formato JSON
			...
			new PNotify({
				title: 'Error!',
				text: respuesta.message || 'Error al guardar el producto',
				type: 'error',
				styling: 'bootstrap3'
			});
		}
		// Compatibilidad con formato antiguo (texto plano)
		else if(datos=="fallo" || respuesta.code=="fallo"){
		...
		} else if(datos=="fallo2" || respuesta.code=="fallo2"){
		...
		}else if(datos=="duplicadoCodigo" || respuesta.code=="duplicadoCodigo"){
		...
		} else if(datos=="exito" || respuesta.code=="exito"){
...
// Función para restar cantidad del inventario de productos
$scope.restarIngresosProducto = function (idProducto, cantidad) {
    // Validar que la cantidad y el idProducto sean válidos
    if (!idProducto || cantidad <= 0) {
        new PNotify({
            title: 'Atención!',
            text: 'Por favor, ingresa un ID de producto válido y una cantidad mayor a 0.',
            type: 'error',
            styling: 'bootstrap3'
        });
        return;
    ...
    $http.post('restar.php', data)
        .success(function (response) {
            ...
            if (response.success) {
                new PNotify({
                    title: 'Éxito!',
                    text: 'Cantidad restada exitosamente.',
                    type: 'success',
                    styling: 'bootstrap3'
                });
            } else {
                new PNotify({
                    title: 'Error!',
                    text: 'Error al restar la cantidad: ' + response.message,
                    type: 'error',
                    styling: 'bootstrap3'
                });
            }
        ...
        .error(function (error) {
            ...
            new PNotify({
                title: 'Error!',
                text: 'Ocurrió un error al restar la cantidad.',
                type: 'error',
                styling: 'bootstrap3'
            });
        });
...
$scope.Confirmaeliminar_ProductoCreditoreXfechas = function(id_credito,id_cliente,fechaInicialPansepare,fechaFinalPansepare,estadoproductos,confirmaeliminacionPlan)
	...
	$scope.buscarfacturaXfechaGeneral = function(idSistemaU,listadoGeneral,value1)
	{
		...
		console.log('idSistemaU', idSistemaU, 'listadoGeneral.fechaInicialFactura', listadoGeneral.fechaInicialFactura,'listadoGeneral.fechaFinalFactura', listadoGeneral.fechaFinalFactura, 'value1', value1);
