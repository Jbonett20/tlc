  ...
  <p>=============================</p>
    <table>
      ...
      $ivaV=0;
        ...
        { 
          ...
          $totalvalor += $respuestadetalle['total_pago'];
          $iva_percent = floatval($respuestadetalle['iva']);
          // Si el total_pago ya incluye IVA, calculamos la porción de IVA con la fórmula: iva = total * iva/(100+iva)
          if ($iva_percent > 0) {
            $ivaVV = ($respuestadetalle['total_pago'] * $iva_percent) / (100 + $iva_percent);
          } else {
            $ivaVV = 0;
          }
          $ivaV += $ivaVV;
          $neto_linea = $respuestadetalle['total_pago'] - $ivaVV; // valor sin IVA por línea
    ...
    $variable_html .= '                 
        <tr class="productos">
          ...
          <td align="right"><strong>$'.number_format($neto_linea).'</strong></td>
          <td align="right"><strong>$'.number_format($ivaVV).' ('.number_format($iva_percent,0).'%)</strong></td>
          <td align="right"><strong>$'.number_format($respuestadetalle['total_pago']).'</strong></td>
        </tr>';
      }
      $subtotal = $totalvalor - $ivaV; // suma de valores sin IVA
